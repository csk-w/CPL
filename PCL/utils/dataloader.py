...
def get_train_loader(args, train_set, num_iters):
    ...
    # === START MODIFICATION: Update collate_fn for new data format ===
    def custom_collate_fn(batch):
        imgs, pids, cams, is_gens, quality_scores, source_paths = zip(*batch)

        imgs = torch.stack(imgs, 0)
        pids = torch.tensor(pids, dtype=torch.long)
        cams = torch.tensor(cams, dtype=torch.long)
        is_gens = torch.tensor(is_gens, dtype=torch.long)
        quality_scores = torch.tensor(quality_scores, dtype=torch.float)

        return imgs, pids, cams, is_gens, quality_scores, source_paths
